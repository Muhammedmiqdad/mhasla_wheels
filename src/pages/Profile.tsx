...
const Profile = () => {
  ...
  // Avoid rendering until user is known (prevents flicker)
  if (!user) return null;

  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-yellow-400 to-yellow-600 px-4 py-8">
      <div className="bg-white shadow-2xl rounded-2xl p-6 sm:p-8 w-full max-w-2xl text-center animate-fade-in">
        {/* Avatar & Basic Info */}
        <div className="mb-6">
              ...
              <tbody>
                {bookings.map((b) => (
                  <tr key={b.booking_code} className="hover:bg-gray-50">
                    ...
                    <td className="p-2 border align-top">{b.pickup_location || "-"}</td>
                    <td className="p-2 border align-top">{b.drop_location || "-"}</td>
                    <td className="p-2 border align-top">{b.journey_type || "-"}</td>
                    <td
